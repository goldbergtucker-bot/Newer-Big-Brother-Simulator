...
let safetyStageIndex = 0;
let safetyGroups = { trashFolder: [], cyberSecurity: [] };
let safetyWinners = { trashFolder: null, cyberSecurity: null };
let firstWeekImmuneIds = [];
let firstWeekImmunitySelector = null;
...
function startNewSeason() {
    ...
    safetyStageIndex = 0; safetyGroups = { trashFolder: [], cyberSecurity: [] }; safetyWinners = { trashFolder: null, cyberSecurity: null }; firstWeekImmuneIds = []; firstWeekImmunitySelector = null;
    evictedHouseguests = [];
    jury = [];
    eventLog = [];
...
function updateGameStageDisplay() {
    const stageMap = {
        ...
        safety1: { name: "Trash Folder", title: "Immunity / Punishment", icon: "📁", description: "Eight Houseguests hunt for folders. The Houseguest without a folder receives the unitard punishment; the special folder winner advances." },
        safety2: { name: "Cyber Security", title: "Immunity / Punishment", icon: "💻", description: "Eight Houseguests compete to spell HOUSEGUEST vertically using blocks while suspended above the competition field." },
        safety3: { name: "Surfing the BB Web", title: "Week 1 Immunity", icon: "🏄", description: "Only the Trash Folder and Cyber Security winners compete. The winner selects eight Houseguests to be safe from the first eviction." },
...
function getCompetitionEvent(kind, week = currentWeek, cycle = currentCycle) {
    ...
    if (kind === "safety") return getCurrentSeasonTemplate()?.safetyCompetitions?.[safetyStageIndex] || null;
    return null;
}

function getSafetyCompetitions() { return getCurrentSeasonTemplate()?.safetyCompetitions || []; }

function initializeSafetyGroups() {
    const active = getActiveHouseguests().slice().sort(() => Math.random() - 0.5);
    safetyGroups = { trashFolder: active.slice(0, 8).map(p => p.id), cyberSecurity: active.slice(8, 16).map(p => p.id) };
}

function getSafetyGroup(key) {
    return (safetyGroups[key] || []).map(findPlayer).filter(Boolean).filter(p => p.status === "Active");
}

function runSafetyCompetition(index) {
    const event = getSafetyCompetitions()[index];
    if (!event) { currentStage = "hoh"; return; }
    if (index === 0 && (!safetyGroups.trashFolder.length || !safetyGroups.cyberSecurity.length)) initializeSafetyGroups();
    if (index < 2) {
        const key = index === 0 ? "trashFolder" : "cyberSecurity";
        const players = getSafetyGroup(key);
        const winner = determineCompetitionWinner(players, event.category || "overall", event.name);
        if (!winner) return;
        safetyWinners[key] = winner.id; winner.competitionWins++; winner.safety = true;
        if (index === 0) {
            const punished = players.filter(p => p.id !== winner.id).sort(() => Math.random() - 0.5)[0];
            if (punished) addEvent(`${getDisplayName(punished)} did not find a folder and receives the unitard punishment.`, "twist");
            addEvent(`${getDisplayName(winner)} found the special folder and advances to Surfing the BB Web.`, "competition");
            currentStage = "safety2";
        } else {
            addEvent(`${getDisplayName(winner)} won Cyber Security and advances to Surfing the BB Web.`, "competition");
            currentStage = "safety3";
        }
        return;
    }
    const finalists = [findPlayer(safetyWinners.trashFolder), findPlayer(safetyWinners.cyberSecurity)].filter(Boolean);
    const winner = determineCompetitionWinner(finalists, event.category || "physical", event.name);
    if (!winner) return;
    firstWeekImmunitySelector = winner.id; winner.competitionWins++;
    const selected = [winner, ...getActiveHouseguests().filter(p => p.id !== winner.id).sort(() => Math.random() - 0.5).slice(0, 7)];
    getActiveHouseguests().forEach(p => { p.safety = false; });
    firstWeekImmuneIds = selected.map(p => p.id);
    selected.forEach(p => p.safety = true);
    addEvent(`${getDisplayName(winner)} won Surfing the BB Web and reprogrammed Week 1.`, "competition");
    addEvent(`${getDisplayName(winner)} selected 8 Houseguests to be immune from the first eviction: ${selected.map(getDisplayName).join(", ")}.`, "twist");
    currentStage = "hoh";
}

function isFirstWeekImmune(player) { return currentWeek === 1 && firstWeekImmuneIds.includes(player?.id); }
...
function getHOHEligible() {
    return getActiveHouseguests().filter(p => p.id !== outgoingHOH && !isFirstWeekImmune(p));
}
...
function isProtectedFromNomination(player) {
    return !!player.safety || isFirstWeekImmune(player) || (player.app === "The Cloud" && !player.appUsed && player.appUsedWeek !== currentWeek);
}
...
function makeNominations() {
    ...
    getActiveHouseguests().forEach(p => { if (p.id !== hoh.id && !isFirstWeekImmune(p) && p.app !== "The Cloud") p.safety = false; });
...
function proceedGame() {
    ...
    switch(currentStage) {
        case "opening": currentStage="safety1"; safetyStageIndex=0; addEvent("Week 1 begins with the three Safety competitions.","competition"); break;
        case "safety1": safetyStageIndex=0; runSafetyCompetition(0); break;
        case "safety2": safetyStageIndex=1; runSafetyCompetition(1); break;
        case "safety3": safetyStageIndex=2; runSafetyCompetition(2); break;
...
function getSaveState() {
    return {
        ...
        currentEvictedPlayer: currentEvictedPlayer?.id || null, outgoingHOH, hackerPower, appStoreHistory, bonusLifeEligible, battleBackCompleted, pendingSecondCycle, safetyStageIndex, safetyGroups, safetyWinners, firstWeekImmuneIds, firstWeekImmunitySelector, finaleWinner: finaleWinner?.id || null,
...
(function installWeeklyPresentation() {
    ...
    function competitionForStage() {
        if (currentStage === "safety1") return getCurrentSeasonTemplate()?.safetyCompetitions?.[0];
        if (currentStage === "safety2") return getCurrentSeasonTemplate()?.safetyCompetitions?.[1];
        if (currentStage === "safety3") return getCurrentSeasonTemplate()?.safetyCompetitions?.[2];
    ...
    renderEventLog = function() {
        const container = $("weeklyTimeline") || $("eventLog");
        if (!container) return;
        const steps = [];
        if (currentWeek === 1 && currentCycle === 1) {
            steps.push({ key:"safety1", label:"SAFETY COMPETITION", icon:"📁", comp:getCurrentSeasonTemplate()?.safetyCompetitions?.[0] });
            steps.push({ key:"safety2", label:"SAFETY COMPETITION", icon:"💻", comp:getCurrentSeasonTemplate()?.safetyCompetitions?.[1] });
            steps.push({ key:"safety3", label:"IMMUNITY", icon:"🏄", comp:getCurrentSeasonTemplate()?.safetyCompetitions?.[2] });
        }
        steps.push({ key:"hoh", label:"HOH", icon:"👑", comp:getCompetitionEvent("hoh") });
        steps.push({ key:"nominations", label:"NOMINATIONS", icon:"🎯" });
        if (currentWeek >= 6 && currentWeek <= 7 && getCurrentSeasonTemplate()?.competitions?.[currentWeek]?.hacker) steps.push({ key:"hacker", label:"H@CKER", icon:"💻", comp:getCompetitionEvent("hacker") });
        steps.push({ key:"pov", label:"POV PICKED PLAYERS", icon:"🎲", comp:getCompetitionEvent("pov") });
        steps.push({ key:"veto", label:"POV", icon:"🏆", comp:getCompetitionEvent("pov") });
        steps.push({ key:"eviction", label:"VETO CEREMONY", icon:"🛡️" });
        steps.push({ key:"evictionReveal", label:"EVICTION VOTE", icon:"🗳️" });
        steps.push({ key:"nextWeek", label:"EVICTION", icon:"🚪" });
        const order=steps.map(x=>x.key);
        let idx=order.indexOf(currentStage);
        if(["finalHOH1","finalHOH2","finalHOH3","finalVote","finale","finished"].includes(currentStage)) idx=steps.length - 1;
        if(idx<0) idx=0;
        container.innerHTML=`<div class="weekly-chain">${steps.map((step,i)=>{
            const done=i<idx, active=i===idx; let summary="Upcoming";
            if(done||active){
                if(step.key==="safety1") summary=safetyWinners.trashFolder?`${getDisplayName(findPlayer(safetyWinners.trashFolder))} advances`:step.comp?.name||"Trash Folder";
                else if(step.key==="safety2") summary=safetyWinners.cyberSecurity?`${getDisplayName(findPlayer(safetyWinners.cyberSecurity))} advances`:step.comp?.name||"Cyber Security";
                else if(step.key==="safety3") summary=firstWeekImmunitySelector?`${getDisplayName(findPlayer(firstWeekImmunitySelector))} selected 8 immune`:step.comp?.name||"Surfing the BB Web";
                else if(step.key==="hoh") summary=currentHOH?`${getDisplayName(findPlayer(currentHOH))} — ${step.comp?.name||"HOH"}`:step.comp?.name||"HOH Competition";
                else if(step.key==="hacker") summary=hackerWinner?`${getDisplayName(findPlayer(hackerWinner))} won`:step.comp?.name||"H@cker Competition";
                else if(step.key==="nominations") summary=nominees.length?nominees.map(getDisplayName).join(" & "):"Nominations";
                else if(step.key==="pov") summary=povPlayers.length?povPlayers.map(getDisplayName).join(", "):step.comp?.name||"POV Players";
                else if(step.key==="veto") summary=povWinner?`${getDisplayName(povWinner)} won`:step.comp?.name||"Power of Veto";
                else if(step.key==="eviction") summary="Veto Ceremony";
                else if(step.key==="evictionReveal") summary=currentEvictionTarget?`${getDisplayName(findPlayer(currentEvictionTarget))} targeted for eviction`:"Eviction Vote";
                else if(step.key==="nextWeek") summary=currentEvictedPlayer?`${getDisplayName(currentEvictedPlayer)} evicted`:"Eviction";
            }
            return `<div class="weekly-chain-step ${done?"is-complete":""} ${active?"is-current":""}"><div class="weekly-chain-node">${step.icon}</div><div class="weekly-chain-content"><div class="weekly-chain-label">${step.label}</div><div class="weekly-chain-summary">${escapeHTML(summary)}</div>${step.comp&&active?`<div class="weekly-chain-comp">${escapeHTML(step.comp.name||"Competition")}</div>`:""}</div></div>${i<steps.length-1?`<div class="weekly-chain-connector ${done?"is-complete":""}"></div>`:""}`;
        }).join("")}</div>`;
    };
    ...
    updateGameStageDisplay = function() {
        ...
        const twistLabels = [];
        if (currentWeek === 1 && template?.safetyCompetitions?.length) twistLabels.push("WEEK 1 IMMUNITY CHAIN");
